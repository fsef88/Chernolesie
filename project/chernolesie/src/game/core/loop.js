...
function frame(now){
 ...
 try{
  ...
  // ============================================================
  if(__vsPrev){const _p=now-__vsPrev;if(_p>2&&_p<40){__vsP[__vsPi++%30]=_p;}}
  __vsPrev=now;
  if(__vsPi>=30&&!(__vsPi%30)){
   // медиана периода развёртки: среднее портит один зависший кадр
   const s=__vsP.slice().sort((a,b)=>a-b);__vsPeriod=s[s.length>>1];
   __vsBase=Math.max(1,Math.round((1000/__vsPeriod)/60));   // 120Гц -> 2, 60Гц -> 1
   if(__vsStride<__vsBase)__vsStride=__vsBase;
  }
  __vsN++;
  if(fpsCap>0){
   ...
   if(__vsN%__vsStride){requestAnimationFrame(frame);return;}
   // Не успеваем в свой шаг — расширяем его, но остаёмся РОВНЫМИ. Кадр, который
   // стабильно приходит через три развёртки вместо двух, глаз принимает; кадр,
   // который приходит то через две, то через четыре, читается как рывок.
   const _want=__vsPeriod*__vsStride, _got=now-lastDraw;
   if(_got>_want*1.4){if(++__vsMiss>8&&__vsStride<4){__vsStride++;__vsMiss=0;__vsOk=0;}}
   else if(++__vsOk>240&&__vsStride>__vsBase){__vsStride--;__vsOk=0;__vsMiss=0;}
   lastDraw=now;
  }else{
   lastDraw=now;
  }
