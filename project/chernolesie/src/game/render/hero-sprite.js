...
// ============================================================
//  v7.42 РАЗБОР ВСЕХ СЕМИ ЛИСТОВ ПО КАДРАМ.
//
//  Листы героев — это не циклы анимации, а НАБОРЫ ПОЗ: художник дал по
//  шестнадцать картинок, и раскладка расставляла их по ходьбе/удару на глаз.
//  Отсюда три вида ошибок, и они были почти у всех. Каждый кадр просмотрен
//  глазами, ниже — что нашлось и что заменено.
//
//  1. ВПЕЧАТАННЫЙ ЭФФЕКТ В ХОДЬБЕ. Кадр удара нарисован вместе с росчерком
//     клинка или выбросом огня. Поставленный в цикл ходьбы, он даёт вспышку
//     на весь кадр у идущего героя — именно это владелец видел как «герой
//     пропал, вместо него картинка».
//       воин   — кадры 6 (22.2% пикселей росчерка) и 8 (8.3%), при 0.5-1.2%
//                у чистых кадров ходьбы 3, 4, 5;
//       лучница— кадр 7 (столб света) и 9 (золотая дуга);
//       огневик— кадры 3 (струя огня), 4 (огненное кольцо), 6 (огненная дуга);
//       знахарка— кадры 3 (посох бьёт молнией) и 4 (кольцо бабочек).
//
//  2. ПРЕДСМЕРТНЫЕ ПОЗЫ В УДАРЕ. У воина в ударе стояли кадры 13 и 14 — это
//     «получил урон, кровь» и «на колене, меч в землю». Герой на замахе
//     показывал то, чем анимация должна КОНЧАТЬСЯ при смерти.
//
//  3. СЦЕНКИ ВМЕСТО ДЕЙСТВИЯ. Часть кадров — не поза, а картинка со своим
//     сюжетом, и в бою она читается как подмена спрайта:
//       знахарка — кадр 13 (сидит с оленёнком) стоял в УДАРЕ,
//                  кадр 6 (собирает цветы в корзину) — в ХОДЬБЕ;
//       стрелок  — кадр 5 (сова на руке) стоял в УДАРЕ,
//                  кадр 13 (стоит рядом с волком) — в ХОДЬБЕ.
//
//  Чисто оказалось у двоих: у Волхва (единственный лист с настоящим циклом
//  ходьбы 1-8) и у Грозника (все кадры — стойки с копьём).
//
//  Правило замены: в ходьбе только нейтральные стойки, в ударе только
//  боевые позы. Длина каждой последовательности сохранена — темп шага
//  считается из неё в drawSprite, и менять его тут нельзя.
// ============================================================
const HERO_LAYOUT={warrior:{idle:[1,2,3,4],walk:[3,3,4,4,5,5,4,4],atk:[7,8,11,12,9,2],n:16},druid:{idle:[1,2,3,4],walk:[1,2,5,7,16,7,5,2],atk:[3,8,12,10,14,15],n:16},rogue:{idle:[1,2,3,4],walk:[13,14,6,1,8,2,10,2],atk:[8,3,9,11,12,13],n:16},shaman:{idle:[9,10],walk:[2,3,4,5,6,7,8,1],atk:[11,12,13,14],ult:[15,16],ultSeq:[15,15,16,16,16,16,14,11,10],atkHit:0.12,n:16},archer:{idle:[1,16,15,14],walk:[1,16,15,14,10,8,10,14],atk:[9,12,2,3,4,1],n:16},ognevik:{idle:[1,2,3,4],walk:[1,2,5,13,16,13,5,2],atk:[13,9,8,3,2,1],n:16},groznik:{idle:[1,2,3,4],walk:[1,2,3,4,5,6,5,4],atk:[2,3,15,14,13,12],n:16}};   // v5.48: живые Знахарка/Страж + новый Грозник
