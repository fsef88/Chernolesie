...
function drawGround(){
 ...
 if(!_gtVar){_gtVar=[];const _hs=[911382323,2608872923,1573669621,377401575];
  for(let v=0;v<4;v++){const c2=document.createElement('canvas');c2.width=_ts;c2.height=_ts;const g2=c2.getContext('2d');g2.drawImage(GT,0,0,_ts,_ts);
   ...
   // v6.13: было 0.30 — вместе с контрастом ушли и ориентиры движения, экран
   // казался неподвижным. 0.16 всё ещё отделяет спрайты от фона, но текстура видна.
   g2.fillStyle='rgba(26,30,24,0.16)';g2.fillRect(0,0,_ts,_ts);
   g2.globalCompositeOperation='saturation';
   g2.fillStyle='hsl(0,72%,50%)';g2.fillRect(0,0,_ts,_ts);   // v6.13: насыщенность снимаем слабее
   g2.globalCompositeOperation='source-over';
