...
function doNavi(){
 ...
 // v7.35: хвост бьёт конусом ЗА СПИНУ (проверка по dot ниже), а зона рисовалась
 // полным кругом да ещё и по взгляду — то есть ровно напротив удара. Угол задаём
 // явно: у этого оружия направление зоны противоположно направлению героя.
 // 0.6 с вместо стандартных 0.42: у хвоста в листе восемь кадров, и на 0.42
 // они пролетают по 52 мс — рук не разглядеть, вспышка читается как мигание.
 // Оружие бьёт раз в 1.3 с, так что зона всё равно гаснет задолго до следующего.
 pulseZone(P.x,P.y,R,'navi',0.6,{arc:half,ang:Math.atan2(fy,fx)});
...
function doKlyuka(){
 ...
 // v7.35: кольцо-волна (_ring) у Клюки убрано — это рисованная кодом окружность,
 // а зону теперь показывает её собственный лист. Поверх листа кольцо читалось
 // как чертёж вокруг рисунка. У Оберега и Зова оно остаётся: своих листов у них
 // пока нет, и там кольцо — единственный признак удара.
 ...
 if(evo)P.invuln=Math.max(P.invuln||0,0.12);
 klyukaCharge=0;
 return true;
}
