...
def despill(fg, alpha):
    ...
    что вылезло выше, придавливается к ней. Белое ядро и мятная заливка
    лежат сильно ниже порога и не трогаются."""
    R, G, B = fg[:, :, 0], fg[:, :, 1], fg[:, :, 2]
    mg = np.minimum(R, B) - G
    core = alpha > 0.9
    if core.sum() < 64:
        return fg
    cap = float(np.percentile(mg[core], 99))
    lim = (G + cap)[:, :, None]
    out = fg.copy()
    out[:, :, 0] = np.minimum(R, lim[:, :, 0])
    out[:, :, 2] = np.minimum(B, lim[:, :, 0])
    return out
