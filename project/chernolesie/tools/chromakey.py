...
def despill(fg, alpha):
    """Убрать невычтенный фон из полупрозрачной каймы.

    Формула альфы выше считает, что у самого эффекта магентовость нулевая.
    У светлых кадров это почти так, но тёмная обводка (#0e3a28 у косы) уходит
    в минус, и на переходном пикселе альфа получается завышенной: фона
    вычитается меньше, чем в нём есть. Остаток — синева по внешнему краю,
    на экране она читается как фиолетовый ореол вокруг эффекта.

    Порог не назначается вручную, а берётся из самого листа: в плотном теле
    (альфа > 0.9) видно, какая магентовость у этого рисунка законная. Всё,
    что вылезло выше, снимается. Белое ядро и мятная заливка лежат сильно
    ниже порога и не трогаются.

    Снимается именно ОБЩИЙ избыток красного и синего, а не каждый канал
    равняется по зелёному. Разница принципиальная: у фиолетового #7a5cff
    красный примерно равен зелёному, а синий законно выше. Равнение по
    отдельности срезало синий и делало из фиолетового листа серый."""
    R, G, B = fg[:, :, 0], fg[:, :, 1], fg[:, :, 2]
    mg = np.minimum(R, B) - G          # фон поднимает КРАСНЫЙ И СИНИЙ разом
    core = alpha > 0.9
    if core.sum() < 64:
        return fg
    cap = max(0.0, float(np.percentile(mg[core], 99)))
    excess = np.clip(mg - cap, 0.0, None)[:, :, None]
    out = fg.copy()
    out[:, :, 0] = np.clip(R - excess[:, :, 0], 0, 255)
    out[:, :, 2] = np.clip(B - excess[:, :, 0], 0, 255)
    return out
